...
@Provider
@Produces("text/occi")
public class OcciCategoriesProvider implements MessageBodyWriter<List<Category>> {
    ...
    public void writeTo(List<Category> categories, Class<?> type, Type genericType, Annotation[] annotations,
      ...
      OutputStream entityStream) throws IOException, WebApplicationException {
        if (categories != null) {
            String categoriesAsOcciString = "";
            for (Category category : categories) {
                categoriesAsOcciString += category.toOcciString() + ",";
            }
            categoriesAsOcciString = categoriesAsOcciString.substring(0, categoriesAsOcciString.length() - 1);
            httpHeaders.add("Category", categoriesAsOcciString);
        }
    }
